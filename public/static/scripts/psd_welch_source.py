...
def welch(sig, Fs):
    wind_length = int(np.round(sig.shape[0]*0.07))
    wind = scipy.signal.hann(wind_length)
    noverlap = round(0.45*wind_length)

    sig = sig - sig.mean()

    nfft = 2**(4+math.ceil(math.log2(abs(sig.shape[0]))))

    F, Pxx = scipy.signal.welch(sig,fs=Fs,window=wind,noverlap=noverlap,nfft=nfft)
